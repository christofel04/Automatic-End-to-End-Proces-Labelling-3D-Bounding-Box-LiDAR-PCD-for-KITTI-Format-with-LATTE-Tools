...
def writeOutput():
	...
	Next_frame_3D_Bounding_Box = frame
	fname = str( int( fname ) + 1 ).zfill( 6 )
	
	Next_frame_3D_Bounding_Box[ "file" ] = Next_frame_3D_Bounding_Box[ "file" ].replace( drivename + "/" + frame[ "filename" ].split( "/" )[ 1 ] , drivename + "/" + fname )
	fh.save_annotation( drivename , fname , Next_frame_3D_Bounding_Box[ "file" ] )
	return str("hi")
